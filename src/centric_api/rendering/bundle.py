...
def print_human_bundle_summary(result: BundleRunResult) -> None:
    ...
    print()
    print("Summary")
    print(f"Items:     {format_count(result.item_count)}")
    print(f"Added:     {format_count(result.added_count)}")
    print(f"Changed:   {format_count(result.changed_count)}")
    print(f"Renamed:   {format_count(result.renamed_count)}")
    print(f"Removed:   {format_count(result.removed_count)}")
    print(f"Unchanged: {format_count(result.unchanged_count)}")
    print(f"Missing:   {format_count(result.missing_count)}")
...
def print_human_bundle_show(run: dict[str, Any], items: list[dict[str, Any]]) -> None:
    ...
    print()
    print("Summary")
    print(f"Items:     {format_count(int(run['item_count'] or 0))}")
    print(f"Added:     {format_count(int(run['added_count'] or 0))}")
    print(f"Changed:   {format_count(int(run['changed_count'] or 0))}")
    print(f"Renamed:   {format_count(int(run['renamed_count'] or 0))}")
    print(f"Removed:   {format_count(int(run['removed_count'] or 0))}")
    print(f"Unchanged: {format_count(int(run['unchanged_count'] or 0))}")
    if items:
        print()
        print("Files")
        rows = items[:50]
...
def print_human_bundle_changelog(comparison: BundleComparison) -> None:
    ...
    print()
    print("Summary")
    print(f"Added:     {format_count(int(summary['added_count'] or 0))}")
    print(f"Changed:   {format_count(int(summary['changed_count'] or 0))}")
    print(f"Renamed:   {format_count(int(summary['renamed_count'] or 0))}")
    print(f"Removed:   {format_count(int(summary['removed_count'] or 0))}")
    print(f"Unchanged: {format_count(int(summary['unchanged_count'] or 0))}")
